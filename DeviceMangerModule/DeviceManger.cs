...
namespace DeviceMangerModule
{
    ...
    public partial  class DeviceManger : DockContent, COM.ZCTT.AGI.Plugin.IPlugin
    {
        ...
        public static List<Device> deviceConnect = new List<Device>();
   ...
   //     Thread deviceConnStateThread;
        ...
        private static  DeviceManger deviceMangerForm;
        ...
        {
            if (deviceMangerForm == null)
            {
                deviceMangerForm = new DeviceManger();
            }
            deviceMangerForm.ShowDialog();
        ...
        {
            if (deviceMangerForm.InvokeRequired)
            {
                deviceMangerForm.Invoke(new deviceConnStateChang(DeviceStateDisp),new object[] {tempDevice,state });
            }
            else
            {
                deviceMangerForm.DeviceStateDisp(tempDevice,state);
            }
        ...
        {
            foreach (ListViewItem tempItem in deviceMangerForm.DeviceListView.Items)
            {
        ...
        private void DeviceListViewItem(string itemName)
        {
          ...
          deviceMangerForm.DeviceListView.Items.Add(item);
        }
        ...
        private void DeviceInfoListViewDisp(string name)
        {
            ...
            if (deviceMangerForm.DeviceInfoListView.InvokeRequired)
            {
                deviceMangerForm.DeviceInfoListView.Invoke(new deviceInfoListChange(DeviceInfoList), new object[] { name });
            }
            else
            {
                DeviceInfoList(name);
        ...
        private void DeviceInfoList(string selectedDeviceName)
        {
            ...
            {
                deviceMangerForm.DeviceInfoListView.Items.Clear();
                deviceMangerForm.DeviceInfoListView.ForeColor = Color.Black;
                if (selectedDevice.PortType == (byte)Global.PortTypeValue.LAN)
                {
                    ...
                    ListViewItem item4 = new ListViewItem("LTE Mode");
                    item4.SubItems.Add(selectedDevice.LteMode);
                    deviceMangerForm.DeviceInfoListView.Items.AddRange(new ListViewItem[] { item1, item2, item3, item4 });
                }
            }
            else
            {
                deviceMangerForm.DeviceInfoListView.Items.Clear();
                deviceMangerForm.DeviceInfoListView.ForeColor = Color.Red;
                ListViewItem exceptionItem = new ListViewItem("Exception");
                exceptionItem.SubItems.Add("Connect failed!");
                deviceMangerForm.DeviceInfoListView.Items.Add(exceptionItem);
            }
        }
              ...
              //  ListViewItem itemPortType = new ListViewItem("InterfaceType");
                if (selectedDevice.PortType == (byte)Global.PortTypeValue.LAN)
                {
                 //   itemPortType.SubItems.Add("LAN");
                    ...
                    item4.SubItems.Add(selectedDevice.LteMode);
                    //this.DeviceInfoListView.Items.AddRange(new ListViewItem[] {itemPortType, item1, item2, item3 });
                    this.DeviceInfoListView.Items.AddRange(new ListViewItem[] {item1, item2, item3, item4 });
                }
                    ...
                    foreach (XmlNode node1 in infoList)
                    {
                        ...
                        if (deviceInfo.InnerText.Length != 0)
                        {
                            ...
                            switch (deviceInfo.Name)
                            {
                                ...
                                case "LteMode": tempDevice.LteMode = tempStr; break;
                                case "OutputDataType":
                                    {
                                        if (tempStr == "IQ")
        ...
        private void UpdateXMLFromMemory()
        {
            try
            {
                ...
                //devices.RemoveAll();
                XmlDeclaration xmldecl = deviceConfig.CreateXmlDeclaration("1.0", "utf-8", null);
                deviceConfig.AppendChild(xmldecl);
                XmlNode devices = deviceConfig.CreateElement("DeviceConfiguration");
                deviceConfig.AppendChild(devices);
                ...
                foreach (Device device in deviceList)
                {
                    ...
                    if (device.PortType == (byte)Global.PortTypeValue.LAN)
                    {
                        ...
                        XmlElement xe9 = deviceConfig.CreateElement("LteMode");
                        xe9.InnerText = device.LteMode;
                        xe.AppendChild(xe2);
                        xe.AppendChild(xe3);
                        xe.AppendChild(xe4);
                        xe.AppendChild(xe9);
        ...
        public static void flashDeviceConnect()
        {
            deviceConnect.Clear();
            foreach (Device device in DeviceManger.deviceList)
            {
                if (device.ConnectionState == (byte)Global.DeviceStateValue.Connecting)
                    deviceConnect.Add(device);
                //Global.GCurrentDevice = device.DeviceName;
            }
            Device temdev;
            for (int i = deviceConnect.Count - 1; i > 0; i--)
            {
                for (int j = 0; j < i; j++)
                {
                    string device1 = deviceConnect[j].IpAddress.Replace(".", "");
                    string device2 = deviceConnect[j + 1].IpAddress.Replace(".", "");
                    if (Convert.ToInt32(device1) > Convert.ToInt32(device2))
                    {
                        temdev = deviceConnect[j + 1];
                        deviceConnect[j + 1] = deviceConnect[j];
                        deviceConnect[j] = temdev;
                    }
                }
            }
        }
